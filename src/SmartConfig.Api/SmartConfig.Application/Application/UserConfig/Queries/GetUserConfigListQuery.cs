...
public class GetUserConfigListQuery : IRequest<List<Core.Models.UserConfig>>
{
    #region Attributes
    ...
    public class Handler : IRequestHandler<GetUserConfigListQuery, List<Core.Models.UserConfig>>
    {
        ...
        public Task<List<Core.Models.UserConfig>> Handle(GetUserConfigListQuery request, CancellationToken cancellationToken)
        {
            ...
            var result = query.SetOrder(request.ConfigOrder).ToList();

            if (request.Options?.ReturnPreferences == false)
                foreach (var userConfig in result)
                {
